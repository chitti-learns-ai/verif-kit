...
function sutContent(id, variant) {
  const p = `../../oracle/${id}/${variant}`;
  // Forward BOTH named and default exports. Real-library reconstructions were
  // CommonJS (module.exports = fn) → `export default`; `export *` alone does not
  // forward a default, so we re-export it explicitly via a namespace binding
  // (which is `undefined`, harmlessly, for named-only modules).
  return (
    `// AUTO-MANAGED by score.mjs — points the SUT at the chosen oracle variant.\n` +
    `// The blind VE imports the public API (signatures live in spec.md) and must\n` +
    `// NOT read this file or anything under oracle/.\n` +
    `import * as _ns from '${p}';\n` +
    `export * from '${p}';\n` +
